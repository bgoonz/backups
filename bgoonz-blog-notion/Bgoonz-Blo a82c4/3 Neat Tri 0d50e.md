# 3 Neat Tricks with Regular Expressions - SitePoint

[https://www.sitepoint.com/3-neat-tricks-with-regular-expressions/](https://www.sitepoint.com/3-neat-tricks-with-regular-expressions/)

I'd like to show you three cunning things you can do with [regular expressions](https://www.sitepoint.com/learn-regex/), that provide neat solutions to some very sticky problems:
...
Originally I tried this with *undefined* characters, and that certainly worked, but it's not safe to assume that any such character will always be undefined (or that the document won't already contain it anyway). Then I discovered that Unicode actually reserves a set of code-points specifically for this kind of thing — so-called [noncharacters](http://en.wikipedia.org/wiki/Mapping_of_Unicode_characters#Noncharacters), which will never be used to define actual characters. A valid Unicode document is not allowed to contain noncharacters, but a program can use them internally for its own purposes.

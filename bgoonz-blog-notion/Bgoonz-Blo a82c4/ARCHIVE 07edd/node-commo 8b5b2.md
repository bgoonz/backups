...
1. If X is a core module,
    ...
    2. STOP
2. If X begins with ‘/'
    1. set Y to be the filesystem root
...
4. let MATCH = PACKAGE_IMPORTS_RESOLVE(X, pathToFileURL(SCOPE), ["node", "require"]) [defined in the ESM resolver](https://nodejs.org/api/esm.md#resolver-algorithm-specification).
...
5. let MATCH = PACKAGE_EXPORTS_RESOLVE(pathToFileURL(DIR/NAME), "." + SUBPATH, `package.json` "exports", ["node", "require"]) [defined in the ESM resolver](https://nodejs.org/api/esm.md#resolver-algorithm-specification).
...
5. let MATCH = PACKAGE_EXPORTS_RESOLVE(pathToFileURL(SCOPE), "." + X.slice("name".length), `package.json` "exports", ["node", "require"]) [defined in the ESM resolver](https://nodejs.org/api/esm.md#resolver-algorithm-specification).
6. RESOLVE_ESM_MATCH(MATCH)
...
3. If EXACT is true,
    1. If the file at RESOLVED_PATH exists, load RESOLVED_PATH as its extension format. STOP
...
- `request`  The module path whose lookup paths are being retrieved.
...
- Type:  `true` if the module is running during the Node.js preload phase.
